...
module.exports = function (source) {
  ...
  // selective arg add
  // TODO test coverage for filename
  if (config.parser) { args.parser = config.parser }
  if (config.filename) { args.filename = config.filename }

  // no need to process extra if its not needed
  if (!config.locals) {
    if (!args.generatorOptions) { args.generatorOptions = {} }
    args.generatorOptions.returnString = true
  }
  ...
  reshape(args)
    ...
    .then((res) => {
      ...
      if (config.locals) {
        ...
        cb(null, `var __runtime = ${runtimeStr}; module.exports = ${res.output.substr(1)}`)
      }
    })
    .catch(cb)
...
function parseOptions (config = [], qs = {}) {
  ...
  if (qs.locals !== undefined) { res.locals = JSON.parse(qs.locals) }
