...
module.exports = function (source) {
  ...
  // options.multi is an array of options priority merged w/ existing
  // if options.multi is present, there is no "normal" compile, only multi
  // each multi must contain a "filename" property which dictates the name of
  // the output.
  // If the locals option is specified, files are emitted
  // If it isn't, the export will be an object with the key being the filename
  // and the value being the template
  ...
  reshape(options)
    ...
    .then((res) => {
      ...
      if (options.locals) {
        ...
        this.emitFile('test.js', 'var foo = "bar"')
